...
// autocomplete busca
add_action( 'init', 'buscaGeralFunc', 2 );
function buscaGeralFunc() {
	if(isset($_GET['buscageral'])){

	    $args = array(
	        'post_type' => array('narrativas', 'perguntas'),
			'posts_per_page' => -1, 
			's' => $_GET['buscageral'],
	    );
	    $q = new WP_Query($args);
	    ...
	    if ($q->have_posts()) {
	        while ($q->have_posts()) {
				...
				$tipo = $postType->labels->singular_name;
...
// Set post likes
function customLikes($postID) {
    $countKey = 'post_views_likes';
    $count = get_post_meta($postID, $countKey, true);
    if($count==''){
        $count = 0;
        delete_post_meta($postID, $countKey);
...
function setLikes(){
	if(isset($_GET['likes_count'])){
		$post_id = intval($_GET['likes_count']);
		$likes_count = customLikes($post_id);
		echo json_encode(array('likes_count' => $likes_count));
		exit;
	}
}
add_action( 'init', 'setLikes', 2 );

// Repeater of Multiple CSV to JSON array of URL
function csvsToJson($csvs){
    if(is_array($csvs)){
        return json_encode(array_map(fn($csv): string => $csv['csv']['url'], $csvs));
    }
    return '[]';
}